...
class SQLQuery {
    ...
    function deleteAll($table,$fieldName,$value) {
        if ($value) {
            $query = 'DELETE FROM '.$table.' WHERE `'.$fieldName.'`=\''.mysql_real_escape_string($value).'\'';
            $this->_result = mysql_query($query, $this->_dbHandle);
            //$this->clear();
            if ($this->_result == 0) {
                /** Error Generation **/
    ...
    function updateRow($table,$id,$data){
        ...
        $query = 'UPDATE '.$table.' SET '.$updates.' WHERE `id`=\''.mysql_real_escape_string($id).'\'';
        $this->_result = mysql_query($query, $this->_dbHandle);
